...
class StableDiffusion:
    ...
    def generate(self, prompt, batch_size=1, num_steps=25, unconditional_guidance_scale=7.5, temperature=1, seed=None,
        ...
        unconditional_context = self.text_encoder.predict_on_batch([self.unconditional_tokens, pos_ids])
    ...
    def get_model_output(self, latent, t, context, unconditional_context, unconditional_guidance_scale, batch_size):
        ...
        return unconditional_latent + unconditional_guidance_scale * (latent - unconditional_latent)
...
def get_models(img_height, img_width, download_weights=True):
    ...
    unet = UNetModel()
    diffusion_model = keras.models.Model([latent, t_emb, context], unet([latent, t_emb, context]))
    # Create decoder
    latent = keras.layers.Input((n_h, n_w, 4))
    decoder = Decoder()
